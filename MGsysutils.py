# MGsysutils.py  - wrappers for windows DiskPart and WMIC utilities
...
# MGtestpatterns() decodes a line of output from windows utilities requests.
...
def MGtestpatterns(line,patternset):                     
...
# newtempfile() - create an application temporary file (this was created for use to create DiskPart runtime script from a list of cmd lines)
...
# class MGwinutils - generic base class for invoking windows utilities via subprocess()
...
class MGwinutils(object):
    ...
    def process_cmd(self):
        ...
        if self.utiltype == MGwinutils.DISKPART:
        ...
        for i in self.reportlines:
            handlerfunc,ngrps,grps= MGtestpatterns( i.decode('utf-8', 'ignore'), self.cmddict['regexes'])
...
# class MGwmic - create instances of various WMIC stoarage device queries. disks, bootdisk, volumes, partitions, computer product info
class MGwmic(MGwinutils):
    (WMIC_BOOTDISK,WMIC_DISKS,WMIC_VOLUMES,WMIC_PARTITIONS,WMIC_CSPRODUCT) = range(0,5)

    def __init__(self, wmicquery ):
        wmic_cmds = dict()
        wmic_cmds[str(MGwmic.WMIC_DISKS)] =  {"cmdline":'wmic  /locale:ms_409 diskdrive list brief /format:csv', 
                                                "regexes": [('^[^,]*,([^,]*),.*PHYSICALDRIVE([0-9]+),.*,([0-9]+),([0-9]+).*$', self.diskshandler),],
                                                "infokeys":['Caption', 'DeviceID','Partitions', 'Size'],
                                                "infoname":'Disks'}
        wmic_cmds[str(MGwmic.WMIC_BOOTDISK)] = {"cmdline":'wmic  /locale:ms409 bootconfig list brief /format:csv ',
        ...
        wmic_cmds[str(MGwmic.WMIC_VOLUMES)] = {"cmdline":'wmic  /locale:ms409 volume list brief /format:csv ',
                                               "regexes":[('^[^,]*,([0-9]+),([0-9]+),([^,]*),([0-9]+),([^,]*),(.*)$',self.volumeshandler),],
                                               "infokeys":['Capacity', 'DriveType', 'FileSystem', 'FreeSpace', 'Label', 'Name'],
                                               "infoname":'Volumes'
                                               }
        wmic_cmds[str(MGwmic.WMIC_PARTITIONS)] = {"cmdline":'wmic  /locale:ms409 partition list brief /format:csv',
        ...
        wmic_cmds[str(MGwmic.WMIC_CSPRODUCT)] = {"cmdline":'wmic  /locale:ms409 csproduct list brief /format:csv',
                                                  "regexes":[('^([^,]*),([^,]*),([^,]*),([^,]*),([^,]*),([^,]*).*$',self.noophandler),],
                                                  "infokeys":['Node',  'Description',  'Identifier', 'Product Name', 'Vendor', 'Version'],
                                                  "infoname":'ComputerProduct'
                                                  }
        super().__init__( MGwinutils.WMIC, wmic_cmds[str(wmicquery)] )
...
class MGdiskpart(MGwinutils):
    (DISKPART_LISTDISKS,DISKPART_DISKS,DISKPART_SETUNIQUEID,DISKPART_GETUNIQUEID,DISKPART_FORMAT) = range(0,5)
    ...
    def __init__(self, diskpartquery, targetnumber=-1, uniqueidstr="" ):
        dp_cmds = dict()
        dp_cmds[str(MGdiskpart.DISKPART_LISTDISKS)] =  {"cmdlist":['list disk',], 
                                                    "regexes": [ ('^.*(Disk)\ ([0-9]+)\s*(.{13})\s*([0-9]*\ (?:GB|MB|KB|TB|B))\s*([0-9]*\ (?:GB|MB|KB|TB|B))\s{2}(.{3})\s{2}(.{1,3}).*$', self.diskdriveunit ),
                                                            ],
                                                    "infokeys":[],
                                                    "infoname":'Disks', 
                                                    "scriptlines":[]}
        dp_cmds[str(MGdiskpart.DISKPART_DISKS)] =  {"cmdlist":['select disk %d'%targetnumber,'detail disk','list partition'], 
                                                    "regexes": [ ('^.*(Disk)\ ([0-9]+)\s*(.{13})\s*([0-9]*\ (?:GB|MB|KB|TB|B))\s*([0-9]*\ (?:GB|MB|KB|TB|B))\s{2}(.{3})\s{2}(.{1,3}).*$', self.diskdriveunit ),
                                                            ('^\s*(.*):\s*(.*)',self.diskitem),
                                                    ...
                                                    "infoname":'Disks', 
                                                    "scriptlines":[]}
        dp_cmds[str(MGdiskpart.DISKPART_GETUNIQUEID)] = {"cmdlist":['rescan','select disk %d'%targetnumber,'uniqueid disk'],
                                                            "regexes":[('^.*(Disk ID: {)([0-9A-F\-]{32})([0-9A-F]{4})}.*$', self.stdguid ), 
                                                                       ('^.*(Disk ID: )([0-9A-F]{8}).*$', self.serno ), ],
                                                            "infokeys":[],
                                                            "infoname":'UniqueID',
                                                            "scriptlines":[] }
        dp_cmds[str(MGdiskpart.DISKPART_SETUNIQUEID)] = {"cmdlist":['select disk %d'%targetnumber,'uniqueid disk id=%s'%uniqueidstr, 'online disk'],
        ...
        dp_cmds[str(MGdiskpart.DISKPART_FORMAT)] = {"cmdlist":['select volume %s'%uniqueidstr[:2],'format fs=%s LABEL=%s %s COMPRESS'%(MGdiskpart.FSTYPE, MGdiskpart.FSLABEL, MGdiskpart.FSMODE)],
                                                    "regexes":[('^.*(Volume)\s([0-9]+)\s*(is the selected volume\.).*$', self.format0 ), 
                                                                ('^\s*([0-9]+ percent completed).*$', self.format1), 
                                                                ('^.*(DiskPart successfully formatted the volume).*$', self.format2 ),
                                                                ],
                                                    "infokeys":[],
                                                    "infoname":'Format',
                                                    "scriptlines":[] }

        super().__init__( MGwinutils.DISKPART, dp_cmds[str(diskpartquery)] )
    ...
    # in an instance of MGdiskpart( MGdiskpart.DISKPART_GETUNIQUEID ... ), call this method to modify the GUID value with a caller-specified offset
    # this returns a guid string which is formatted for use in a new MGdiskpart.DISKPART_SETUNIQUEID instance/ operation
...
if __name__ == '__main__' :
    ...
    # note that Format example will fail due to illegal drive spec in the instantiation. it FORMATs the disk! the code with care 
    print('** MGSysUtils.py TEST PROGRAM **')
    ...
    diskinfx = MGdiskpart(MGdiskpart.DISKPART_LISTDISKS)
    disk_numbers = []
    for i in diskinfx.diskdicts:
        print('----- Disk ----->  ', i)
        diskinf = MGdiskpart(MGdiskpart.DISKPART_DISKS, int(i['Disk ###']))
        ...
        for j in diskinf.partitiondicts:
            print('-----Partitions', j)
        diskuuid = MGdiskpart(MGdiskpart.DISKPART_GETUNIQUEID, int(i['Disk ###']))
        print('-----Disk GUID', diskuuid.GUID)

    # WARNING test the GUID update - this is very specific to MY SYSTEM/ DISK #2!!
    diskuuid2 = MGdiskpart(MGdiskpart.DISKPART_GETUNIQUEID, 2)
    if 2 in disk_numbers:
        ...
        if False: # DISABLE THE GUID modification so noone messes up a system configuration
            diskuuid3 = MGdiskpart(MGdiskpart.DISKPART_SETUNIQUEID, 2, newguidstr)
        else:
            print("DISABLED TEST CODE:   MGdiskpart(MGdiskpart.DISKPART_SETUNIQUEID, 2, newguidstr)")

    # WARNING *** FORMAT A VOLUME *** THIS WILL WIPE ANY DATA FROM THE TARGET DISK!!
    diskfmt = MGdiskpart(MGdiskpart.DISKPART_FORMAT, 2, "----invalid text here ---valid drive letter target required F:")
    ...
    # the boot disk info
    lb, Wbootinf = MGwmic(MGwmic.WMIC_BOOTDISK).mgwu_getinfo()
    for i in Wbootinf :
        print('Boot',i)

    # all the disks
    ld, Wdiskinf = MGwmic(MGwmic.WMIC_DISKS).mgwu_getinfo()
    for i in Wdiskinf :
        print('disk',i)

    # all the volumes
    lv, Wvolinf = MGwmic(MGwmic.WMIC_VOLUMES).mgwu_getinfo()
    for i in Wvolinf :
        print('volume',i)

    # all the partitions
    lp1, Wpartinf = MGwmic(MGwmic.WMIC_PARTITIONS).mgwu_getinfo()
    for i in Wpartinf :
        print('partition',i)

    # the computer product info
    lp2, Wprodinfo = MGwmic(MGwmic.WMIC_CSPRODUCT).mgwu_getinfo()

    print('PRODUCT INFORMATION', Wprodinfo )
    print('** MGSysUtils.py TEST PROGRAM COMPLETE**')
